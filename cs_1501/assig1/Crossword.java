...
public class Crossword
{
	...
	public Crossword(String dictType, String boardName, boolean fast) throws FileNotFoundException
	{
		...
		solutions = 0;
		if(fast)	// passed from args[2] flag
			solveTileFaster(0, 0);
		else
			solveTile(0, 0);
	...
	public static void main(String[] args) {
		if (args.length < 2)
		{
			System.err.println("Usage: java Crossword <dictionary_type> <board_file> [optional]<\"fast\">");
			System.exit(1);
		}
		//if the third command line argument is "fast," run the improved algorithm
		boolean fastVersion = false;		
		if(args.length == 3 && args[2].equalsIgnoreCase("fast"))	
			fastVersion = true;										
		try
		{
			new Crossword(args[0], args[1], fastVersion);
		}
	...
	//This is the original recursive backtracking method which fills the board
	//r is the current row and c is the current column
	private void solveTile(int r, int c)
	{
		...
		if(currTile == '+')						//generate solutions for '+' tiles
		{
			for(int i = 0; i < 26; i++)			//try all letters in sequence
			{
				...
				if(c == n - 1)
					solveTile(r + 1, 0);	//recursive case move to next row
				else
					solveTile(r, c + 1);	//recursive case move down the row
		...
		else	//currTile is a character which must be used as is (includes '-')
		{
			...
			{
				rows[r].deleteCharAt(c);
				return;					//backtrack currTile breaks row
			}
			...
			if(!verifyBuilder(columns[c]))
			{
				rows[r].deleteCharAt(c);
				columns[c].deleteCharAt(r);
				return;					//backtrack currTile breaks column
			}
			if(c == n - 1)
				solveTile(r + 1, 0);	//recursive case move to next row
			else
	...
	//This is the improved recursive backtracking method which fills the board
	//r is the current row and c is the current column
	//@ return the number of tiles left to backtrack
	private int solveTileFaster(int r, int c)
	{
		if(r == n)	//solution has been found, process result
		{
			if(solutions % 10000 == 0)
		...
		if(currTile == '+')						//generate solutions for '+' tiles
		{
			for(int i = 0; i < 26; i++)			//try all letters in sequence
			{
				...
				if(c == n - 1)
				{
					...
					tilesToBacktrack = solveTileFaster(r + 1, 0);
				}
				else
				{
					//recursive case move down the row
					tilesToBacktrack = solveTileFaster(r, c + 1);
				}
				rows[r].deleteCharAt(c);	//backtrack here if no letter works
				columns[c].deleteCharAt(r);
